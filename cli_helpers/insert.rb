module InsertCli
  def parse_insert(arr)
    ...
    insert_cols = []
    player_data_columns = %w[name year_start year_end position height weight birth_date college]
    players_columns = %w[Player height weight college born birth_city birth_state]

    ins_hash['VALUES'] = ins_hash['VALUES'].split(/\(|,|\)/)[1..-1].map { |i| i.gsub(/'/, '') }

    if ins_hash['INSERT INTO'].length > 13
      ins_hash['INSERT INTO'], insert_cols = ins_hash['INSERT INTO'].split(/ \(|,|\)/).partition.with_index do |_, i|
        i == 0
      end
    ...
    if ins_hash['INSERT INTO'] == 'player_data' && ins_hash['VALUES'].length == player_data_columns.length
      ...
      @insert_query[:values] = Hash[player_data_columns.map do |i|
                                      [i, '']
                                    end ].merge(insert_cols.zip(ins_hash['VALUES']).to_h)
